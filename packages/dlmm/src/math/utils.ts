...
export class MathUtil {
  ...
  static checkUnsignedSub(n0: BN, n1: BN): BN {
    const n = n0.sub(n1)
    if (n.isNeg()) {
      throw new DlmmPairsError('Unsigned integer sub overflow', MathErrorCode.UnsignedIntegerOverflow)
    }
    return n
  }
  ...
  static checkMul(n0: BN, n1: BN, limit: number): BN {
    ...
    if (this.isOverflow(n, limit)) {
      throw new DlmmPairsError('Multiplication overflow', MathErrorCode.MulOverflow)
    }
    return n
  }
  ...
  static checkMulDivFloor(n0: BN, n1: BN, denom: BN, limit: number): BN {
    if (denom.eq(ZERO)) {
      throw new DlmmPairsError('Devide by zero', MathErrorCode.DivideByZero)
    }
    const n = n0.mul(n1).div(denom)
    if (this.isOverflow(n, limit)) {
      throw new DlmmPairsError('Multiplication div overflow', MathErrorCode.MulDivOverflow)
    }
    return n
  }
  ...
  static checkMulShiftRight(n0: BN, n1: BN, shift: number, limit: number): BN {
    ...
    if (this.isOverflow(n, limit)) {
      throw new DlmmPairsError('Multiplication shift right overflow', MathErrorCode.MulShiftRightOverflow)
    }
    return n
  }
  ...
  static checkMulShiftLeft(n0: BN, n1: BN, shift: number, limit: number): BN {
    ...
    if (this.isOverflow(n, limit)) {
      throw new DlmmPairsError('Multiplication shift left overflow', MathErrorCode.MulShiftLeftOverflow)
    }
    return n
  }
