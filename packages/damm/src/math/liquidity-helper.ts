...
export function getLiquidityAndCoinYByCoinX(
  ...
  if (coinInVal.lessThanOrEqualTo(0)) {
    throw new DammpoolsError('coinInVal is less than zero', MathErrorCode.InvalidCoinAmount)
  }
...
export function getCoinXYForLiquidity(
  ...
  if (liquidity.lessThanOrEqualTo(0)) {
    throw new DammpoolsError("liquidity can't be equal or less than zero", MathErrorCode.InvalidLiquidityAmount)
  }

  if (reserveInSize.lessThanOrEqualTo(0) || reserveOutSize.lessThanOrEqualTo(0)) {
    throw new DammpoolsError('reserveInSize or reserveOutSize can not be equal or less than zero', MathErrorCode.InvalidReserveAmount)
  }
