...
export class Trade<CInput extends Coin, COutput extends Coin> {
  ...
  /**
   * Execute only the routing swaps without building trade object or settling.
   * Use this when you have a custom trade object (e.g., from Ferra's start_swap).
   *
   * @param params.tx - Transaction to append move calls to
   * @param params.tradeObject - Custom trade object from external source
   * @param params.client - SuiClient for pyth price feeds
   */
  public async swapRoutes(params: {
    tx: Transaction;
    ...
    // Execute swap for each route (start_routing -> swaps -> finish_routing)
    for (const route of this.routes) {
      route.swap(tradeObject, new Percent(this.slippage, BPS), pythMap)(tx);
    }
  }
