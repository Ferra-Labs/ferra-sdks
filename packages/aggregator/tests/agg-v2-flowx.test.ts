...
/**
 * Test script for FlowX Aggregator integration
 *
 * Flow:
 * 1. Initialize SDK with FlowX provider using initMainnetAggV2SDK
 * 2. Get quote for SUI -> USDC swap via FlowX
...
export async function main() {
    ...
    // ============ Initialize SDK ============
    // Use initMainnetAggV2SDK with FlowX provider
    const sdk = initMainnetAggV2SDK(AggProvider.FLOWX, wallet, {
        providers: {
            flowx: {
                maxHops: 3,
            }
    ...
    console.log('📦 SDK initialized with provider:', AggProvider.FLOWX)
    ...
    try {
        // ============ Step 1: Get Quote ============
        console.log('\n📊 Step 1: Fetching quote from FlowX...')
        ...
        if (!quote) {
            throw new Error('No quote found for SUI -> USDC swap via FlowX')
        }
        ...
        // FlowX specific quote info
        const flowxQuote = quote.quote as any
        if (flowxQuote?.routes) {
            console.log('- Routes Count:', flowxQuote.routes.length)
            flowxQuote.routes.forEach((route: any, idx: number) => {
                ...
                if (route.paths) {
                    ...
                    route.paths.forEach((path: any, pathIdx: number) => {
                        console.log(`      Path ${pathIdx + 1}: ${path.source || 'unknown'} (${path.tokenIn?.split('::').pop()} -> ${path.tokenOut?.split('::').pop()})`)
                    })
                }
            })
        }
        ...
        // Set gas budget
        tx.setGasBudget(50_000_000) // 0.05 SUI (FlowX may need more gas)
...
// bun ./tests/agg-v2-flowx.test.ts