...
import { FerraClmmSDK } from '../../src/sdk'
...
export async function mintAll(sdk: FerraClmmSDK, sendKeypair: Ed25519Keypair, faucet: Package, funName: string) {
...
export function buildSdk(sdkEnv: SdkEnv = currSdkEnv): FerraClmmSDK {
  currSdkEnv = sdkEnv
  switch (currSdkEnv) {
    case SdkEnv.mainnet:
      return SDK
...
export async function buildTestPool(sdk: FerraClmmSDK, poolObjectId: string) {
  const pool = await sdk.Pool.getPool(poolObjectId)
  console.log('buildPool: ', pool)
  return pool
}

export async function buildTestPosition(sdk: FerraClmmSDK, posObjectId: string) {
