...
import { FerraClmmSDK, CoinAsset, CoinAssist, TransactionUtil, CoinProvider, PathProvider } from '../src'
...
export async function execTxReturnRes(sdk: FerraClmmSDK, payload: Transaction) {
  const { simulationAccount } = sdk.sdkOptions
  const simulateRes = await sdk.fullClient.devInspectTransactionBlock({
    transactionBlock: payload,
    sender: simulationAccount.address,
