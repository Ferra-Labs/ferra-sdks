...
export class TxBlock {
  ...
  transferSuiToMany(recipients: string[], amounts: number[]) {
    if (recipients.length !== amounts.length) {
      throw new LimitError('The length of recipients and amounts must be the same', LimitErrorCode.InvalidRecipientAndAmountLength)
    }

    for (const recipient of recipients) {
      if (!checkValidSuiAddress(recipient)) {
        throw new LimitError('Invalid recipient address', LimitErrorCode.InvalidRecipientAddress)
      }
    }
