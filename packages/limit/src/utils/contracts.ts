...
export function isSuiCoin(coinType: string) {
  return normalizeCoinType(coinType) === GAS_TYPE_ARG
}
  ...
  try {
    ...
    if (generics) {
      ...
      structTag.type_arguments = structTag.type_arguments.map((item) => {
        return isSuiCoin(item) ? item : extractStructTagFromType(item).source_address
      })
    ...
    const isSui = _type === GAS_TYPE_ARG || _type === GAS_TYPE_ARG_LONG

    const structTag: SuiStructTag = {
      full_address: _type,
      address: isSui ? '0x2' : normalizeSuiObjectId(parts[0]),
      module: parts[1],
      name: parts[2],
      type_arguments: [],
      source_address: '',
