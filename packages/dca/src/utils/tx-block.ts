...
export class TxBlock {
  ...
  transferSuiToMany(recipients: string[], amounts: number[]) {
    if (recipients.length !== amounts.length) {
      throw new DcaError('The length of recipients and amounts must be the same', DcaErrorCode.InvalidRecipientAndAmountLength)
    }

    for (const recipient of recipients) {
      if (!checkValidSuiAddress(recipient)) {
        throw new DcaError('Invalid recipient address', DcaErrorCode.InvalidRecipientAddress)
      }
    }
