import { DcaError, DcaErrorCode, DETAILS_KEYS } from '../errors/errors'
import { FerraDcaSDK, SdkOptions } from '../sdk'
import { DcaConfigs } from '../types'
...
export function getPackagerConfigs(config: SdkOptions['dca']): DcaConfigs {
  if (!config.config) {
    throw new DcaError("INVALID_CONFIG", DcaErrorCode.BuildError, {
